...
// initialize elements and data
	...
	function showCalendar(month, year){
		...
		const days = [];
		$.ajax({
			url : BASEURL+'mutabaah/getCalendar/'+monthly+'/'+year,
			type: "GET",
			async: false,  
			headers: {
		...
		success: function(data){
			tgl = data.data;
			hari = new Date(tgl[0]['tgl_mutabaah']).getDate();
			arr.push(hari);
			for(i = 0; i < tgl.length; ++i){
				hari = new Date(tgl[i]['tgl_mutabaah']).getDate();
				if($.inArray(hari, arr) < 0){
					arr.push(hari);
				} 
			}
		},
			...
			for(let j = 0; j < 7; j++){
				if(i === 0 && j < firstDay){
					...
					if(arr.includes(date)){
						cell = $(`<div class="day mutabaah-day-block" onclick="showdetail()" data-day="${date}" data-month="${monthly}" data-year="${year}"><p class="mutabaah-day">${date}</p></div>`); 
					}
					// append cell to the row
					row.append(cell);
					date++;
...
// year selector
	yearSpinner.find('.fa-angle-up').on('click', rollDown);

	yearSpinner.find('.fa-angle-down').on('click', rollUp);
